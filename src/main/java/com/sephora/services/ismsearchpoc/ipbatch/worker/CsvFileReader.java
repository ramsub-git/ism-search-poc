...
public class CsvFileReader<R> implements WorkItemReader<String, R> {
    @Autowired
    @Qualifier("CSV_PARSE")
    private ContextualStepGuard<SkulocRecord> csvParseGuard;
    ...
    public CsvFileReader(BlobContainerClient blobClient, 
        ...
        public List<R> readWorkItem(String fileName, ExecutionContext context) throws Exception {
            return csvParseGuard.execute(
                    () -> {
                        try {
                            return guardedReadWorkItem(fileName, context);
                        } catch (Exception e) {
                            // wrap as unchecked so the lambda doesn't throw a checked exception
                            throw new RuntimeException("Error readWorkItem", e);
                        }
                    },
                    ""
            );
        ...
        public List<R> guardedReadWorkItem(String fileName, ExecutionContext context) throws Exception {
